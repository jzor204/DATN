...
func (r *ProjectRepository) ListManagers(ctx context.Context, projectID uint) ([]*domain.ProjectMember, error) {
	var rows []projectMemberModel

	if err := r.db.WithContext(ctx).
		Where("project_id = ? AND role_in_project IN ?", projectID, []string{
